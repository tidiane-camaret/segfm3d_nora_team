...
singularity_path = args.docker_folder_path
        ...
        for case in tqdm(test_cases):
            ...
            for it in range(n_clicks + 1): # + 1 due to bbox pred at iteration 0
                ...
                if torch.cuda.is_available(): # GPU available
                    cmd = 'singularity exec --nv -B $PWD/inputs/:/workspace/inputs/,$PWD/outputs/:/workspace/outputs/ {} /bin/bash -c "cd /workspace && sh predict.sh"'.format(join(singularity_path, docker))
                else:
                    cmd = 'singularity exec -B $PWD/inputs/:/workspace/inputs/,$PWD/outputs/:/workspace/outputs/ {} /bin/bash -c "cd /workspace && sh predict.sh"'.format(join(singularity_path, docker))
